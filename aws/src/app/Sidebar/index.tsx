...
import { Menu , LucideIcon, Layout, Archive, Clipboard, User, CircleDollarSign} from 'lucide-react';
    ...
    return (
        <Link href={href}>
            ...
            >
                ...
                <span className={`${isCollapsed ? "hideen": "block"} font-medium text-gray-700`}>
                    {label}
                </span>
...
const Sidebar = () =>{
    ...
    const isSidebarCollapsed = useAppSelector(
        (state) => state.global.isSidebarCollpased
    )
    ...
    return( <div className={sidebarClassNames}>
        <div className={`flex gap-3 justify-between md:justify-normal items-center pt-8 ${isSidebarCollapsed ? "px-5" : "px-8"}`}>
            ...
            <h1 className={`${isSidebarCollapsed ? "hidden" : "block"}font-extrabold text-2x1`}>
                EDSTOCK
            </h1>
        ...
        <div className='flex-grow mt-8 '>
            ...
            <SidebarLink href='/settings' icon={Layout} label='settings' isCollapsed={isSidebarCollapsed}/>
            <SidebarLink href='/expenses' icon={CircleDollarSign} label='Expenses' isCollapsed={isSidebarCollapsed}/>

        </div>
        <div className={`${isSidebarCollapsed? "hidden" : "block"} mb-10`}>
            <p className='text-center text-xs text-gray-500'>&copy; 2024 Educstock</p>
        </div>
        </div>
